...
12. Click `扫描大文件夹并分类` to choose a messy source folder and a destination folder. The app will move files into `Comics`, `Text`, `Audio`, `Video`, `Archives`, and `Other`.
...
12. 点 `扫描大文件夹并分类`，先选择杂乱的大文件夹，再选择目标文件夹，应用会自动移动到漫画、文本、音频、视频等分类目录。
...
The built-in comic reader supports quick page turning, first/last page buttons, zoom, fit-to-page, fit-to-width, actual-size mode, and pure reading mode.

内置漫画阅读器支持快速翻页、第一页/最后一页、缩放、适应页面、适应宽度、原始大小和纯阅读模式。
